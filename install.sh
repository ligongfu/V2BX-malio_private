...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        # 使用0175angela的V2BX-malio仓库
        last_version=$(curl -Ls "https://api.github.com/repos/0175angela/V2BX-malio/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -q -N --no-check-certificate -O /usr/local/V2bX/V2bX-linux.zip https://github.com/0175angela/V2BX-malio/releases/download/${last_version}/V2bX-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/0175angela/V2BX-malio/releases/download/${last_version}/V2bX-linux-${arch}.zip"
...
EOF
    ...
    if [[ ! -f /etc/V2bX/config.json ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}V2bX 可能启动失败，请稍后使用 V2bX log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/0175angela/V2BX-malio${plain}"
        fi
        first_install=false
    fi
